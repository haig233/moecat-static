...
//document.getElementById("filelist").innerHTML='载入中...';
    document.getElementById("showfl").style.display = 'none';
    document.getElementById("hidefl").style.display = 'block';
    jQuery.get("viewfilelist.php?", {id: torrentid}, function (data) {
        document.getElementById("filelist").innerHTML = data;
        jQuery(document.getElementById("filelist")).slideDown();
...
function getusertorrentlistajax(userid, show, id) {
    ...
    if (klappText.style.display == 'none') {
        //klappText.style.display = '';
        jQuery.get("getusertorrentlistajax.php?", {userid: userid, type: show}, function (data) {
            klappText.innerHTML = data;
            jQuery(klappText).slideDown(1000);
        }, "html");
        klappBild.className = 'minus';
    } else {
        //klappText.style.display = 'none';
...
function get_external_data() {
	...
	let external_api_base_url = "https://api.mocat-gen.workers.dev";
...
//$('#mainmenu li .normalMenu').each(function(){ $(this).before($(this).clone().removeClass().addClass('active')); });
    $('#mainmenu li').hover(
        function () {
            $(this).find('.active').stop(true, true).animate({marginTop: '0px'}, 200);
            $(this).find('.active2').stop(true, true).slideDown(200);
        },
        function () {
            $(this).find('.active').animate({marginTop: '-24px'}, 200);
            $(this).find('.active2').slideUp(200);
        }
    )
});
    ...
    if ($('#nav').length) {
        ...
        $(window).scroll(function () {
            if ($(this).scrollTop() > _defautlTop) {
                if (!navfix) {
                    $('#nav').css({
                        '-webkit-box-shadow': '0 0 20px #' + LinkEndColor,
                        'position': 'fixed',
                        'top': 0,
                        'left': (_defautlLeft - $(window).scrollLeft()),
                        'z-index': 99999
                    });
                    navfix = true;
                }
...
$(function () {
    $('.transitionpic').hover(
        function () {
            $(this).css({'-webkit-box-shadow': '0 0 20px #' + LinkEndColor})
        },
        function () {
