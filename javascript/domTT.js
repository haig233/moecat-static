...
function domTT_activate(in_this, in_event) {
    ...
    // non active event (make sure we were passed a string id)
        ...
        if (typeof (in_this) != 'object' && !(owner = domTT_tooltips.get(in_this))) {
            ...
            //jQuery(owner).slideUp();//chenzhuyu
    ...
    if (options.has('statusText')) {
        try {
            window.status = options.get('statusText');
        } catch (e) {
        }
    }
    ...
    if (!options.has('content') || options.get('content') == '' || options.get('content') == null) {
        if (typeof (owner.onmouseout) != 'function') {
            owner.onmouseout = function (in_event) {
                domTT_mouseout(this, in_event);
            };
        }
...
function domTT_create(in_options) {
    ...
    if (in_options.get('trail') && typeof (tipOwner.onmousemove) != 'function') {
        tipOwner.onmousemove = function (in_event) {
            domTT_mousemove(this, in_event);
        };
    }

    if (typeof (tipOwner.onmouseout) != 'function') {
        tipOwner.onmouseout = function (in_event) {
            domTT_mouseout(this, in_event);
        };
    }
...
function domTT_show(in_id, in_event) {
    ...
    if (status == 'pending') {
        ...
        //jQuery(tipObj).slideDown();//chenzhuyu
        ...
        if (status == 'pending') {
            ...
            if (tooltip.get('closeAction') == 'hide') {
                ...
                if (fade != 'neither') {
                    ...
                    //jQuery(tipObj).slideUp();//chenzhuyu//chenzhuyu
                }
            } else {
...
function domTT_runDeactivate(args) {
    domTT_deactivate(args[0]);
}

function domTT_runShow(args) {
    domTT_show(args[0], args[1]);
}
